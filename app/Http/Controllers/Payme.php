...
class Payme extends Controller
{
     ...
     public function indexxx(Request $req)
    {
        if ($req->method == "CheckPerformTransaction") {
            if (empty($req->params['account'])) {
                ...
                $order = User::where('id', $a['order_id'])->first();
                if (empty($order)) {
                    $response = [
                        'id' => $req->id,
                        'error' => [
    ...
    public function index(Request $request)
    {
        if ($request->method === "CheckPerformTransaction") {
            ...
            } elseif ($request->params['account']['user_id'] === null) {
                $response = [
                    "id" => $request->id,
                    "error" => [
                        "code" => 31050,
        ...
        if ($request->method === "CreateTransaction") {
            ...
            $id = $request->id;
            $user_id = $request->params['account']['user_id'];
            $amount = $request->params['amount'];

            $transaction = Transaction::create([
                'paycom_time' =>  $request->params['time'],
                'paycom_time_datetime' => $request->params['time'],
                'user_id' => $request->params['account']['user_id'],  // Make sure user_id is set correctly
        ...
        if ($request->method === "CheckTransaction") {
            ...
            $transaction = Transaction::where('paycom_transaction_id', $id)->first();

             $transaction->state = 2;
             // $milliseconds = round(microtime(true) * 1000);
              $response = [
                'result' => [
                    'perform_time' =>  $transaction->paycom_time,
                    'create_time' =>  $transaction->paycom_time,
                    'transaction' => intval( $transaction->id ),
                    'cancel_time' => 0,
                    'state' =>  $transaction->state,
                    'reason' => null
                ],
